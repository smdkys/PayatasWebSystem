...
if(isset($_POST["submit"])){
    ...
    $sql = "INSERT INTO `livelihoodoffsiteform`(`date`, `time`, `program_type`, `participants`, `location`, `fullname`, `contact`, `photo`, `photo2`) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
...
<head>
    ...
    <title>Livelihood - Offsite</title>
    ...
    <script>
        function openModal() {
            ...
            if (!date || !time || !program_type || !participants || !location || !fullname || !contact || !file1 || !declaration) {
                alert("Please fill in all required fields and upload a request letter.");
                return;
            }
